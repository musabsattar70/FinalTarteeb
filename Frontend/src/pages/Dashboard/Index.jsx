  ...
  useEffect(() => {
    const fetchInstructors = async () => {
      const response = await fetch(
        import.meta.env.VITE_API_URL + "/api/admin/GetAllAdminInfo",
        {
          method: "POST",
          headers: {
      ...
      if (selectedAlphabetChange) {
        setSelectedAlphabetInstructor(
          data.Data.filter(
            (instructor) => instructor.FirstName[0] == selectedAlphabetChange
          )
        );
      }
    };
  ...
  return (
    <section className="homepage_section">
      ...
      <main>
        ...
        <InstructorsList
          instructors={
            selectedAlphabetChange ? selectedAlphabetInstructor : instructors
          }
        />
      </main>
    </section>
