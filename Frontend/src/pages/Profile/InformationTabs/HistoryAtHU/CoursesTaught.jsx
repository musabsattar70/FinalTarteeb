...
export const CoursesTaught = () => {
  ...
  const fetchCourseList = async () => {
    const response = await fetch(
      import.meta.env.VITE_API_URL + "/api/teaching-history/" + id
    );
  ...
  const handleAddCourse = async () => {
    ...
    const response = await fetch(
      import.meta.env.VITE_API_URL + "/api/teaching-history",
      {
        method: "POST",
        headers: {
        ...
        {!selectedCourse &&
          Object.keys(groupedBySemester).map((key, index) => (
            <div key={index}>
              ...
              <div className="hide_on_init" id={key.replace(" ", "")}>
