...
export const AtHabibUniversity = () => {
    ...
    const fetchSupervisions = async () => {
        const response = await fetch(
            `http://localhost:7075/api/supervision/${id}`
        );
        const data = await response.json();
        setSupervisionsListState(data.Data?.filter((item) => item.Location == "Habib University"));
    };
    ...
    const handleAddSupervision = async () => {
        const supervisionName =
            document.getElementById("supervision_name").value;
        const supervisionAuthors = document.getElementById(
            "supervision_authors"
        ).value;
        const supervisionDate =
            document.getElementById("supervision_date").value;
        // const supervisionLocation = document.getElementById(
        //     "supervision_location"
        // ).value;

        const response = await fetch(`http://localhost:7075/api/supervision/`, {
            method: "POST",
            headers: {
    ...
    const handleDelete = async (index) => {
        ...
        const response = await fetch(
            `http://localhost:7075/api/supervision/${index}`,
            {
                method: "DELETE",
            }
        );
