...
export const CommitteeMemberships = () => {
    ...
    const fetchCommitteeList = async () => {
        const response = await fetch(
            `http://localhost:7075/api/committee-membership/${id}`
        );
    ...
    const handleAddCommitteeDetails = async () => {
        ...
        const response = await fetch(
            `http://localhost:7075/api/committee-membership/`,
            {
                method: "POST",
                headers: {
            ...
            `http://localhost:7075/api/committee-membership/`,
            {
                ...
                body: JSON.stringify({
                    committeeMembershipId:
                        selectedCommittee.CommittieeMembershipId,
                    committeeName: committeeName,
                    designation: committeeDesignation,
                    termFrom: termFromState,
                    termTo: termToState,
    ...
    const handleDelete = async(index) => {
        const response = await fetch(
            `http://localhost:7075/api/committee-membership/${index}`,
            {
                method: "DELETE",
            }
        );
