...
export const Externel = () => {
  ...
  const fetchWorkshops = async () => {
    const response = await fetch(
      `${import.meta.env.VITE_API_URL}/api/workshop/${id}`
    );
  ...
  const handleAddWorkshop = async () => {
    ...
    const location = document.getElementById("workshop_location").value;
  ...
  const handleDelete = async (index) => {
    ...
    const response = await fetch(
      `${import.meta.env.VITE_API_URL}/api/workshop/${index}`,
      {
        method: "DELETE",
      }
    );
